...
double admin :: MinScore(int pname)
{
	...
	for(uint i=0;i<problems.size();i++)
	{
	    ...
	    int grpmin = *min_element(problems.at(i).begin(),problems.at(i).end());
	    if(grpmin < min)
	    {
		gindex=i;
		min=grpmin;
...
void admin::AfterExit()
{
	...
	if(poutput.stripWhiteSpace() ==  vcases.at(casei).output.stripWhiteSpace() )
	{
		OP("Correct :-)");
	...
	else
	{
		if(!perfect)
		{
			OP("\n\n");
			if(no_wrong_cases==1)
				OP("Output for a Test Case was WRONG !!!\n\n");
			else
			{
				    if(no_wrong_cases == vcases.size())
					OP("Output for all the Test Cases were WRONG !!!\n\n");
				    else
					OP("Output for "+QString::number(no_wrong_cases)+" Test Cases were WRONG !!!\n\n");
			}	    
			double prev_score=score ;
			score = score*MinScore(currprob.toInt())/currsubscore ;
			OP("Total Score For "+currprob+" Point Problem : " + QString::number(score)) ;
			if(score != prev_score)
			    OP(" ( Instead Of "+QString::number(prev_score)+" )");
			OP("\n");
		}	
		else
...
void admin::NextTest()
{
	...
	err=false;
	if(vcases.at(casei).input.stripWhiteSpace()=="")
	    vcases.at(casei).input="0" ;
	if(p->launch( vcases.at(casei).input))
	{
		OP("\n\nTest Case " + QString::number(casei+1) + " : ");
		t->start(timelimit,false);
		s_time=ToMSeconds(QTime::currentTime());
...
void admin :: Update()
{
	...
	Log->setText("Server Log : <br><br>"+ReadFile(path+"log.txt"));
	QString teamlist ; 
...
{
	if(QMessageBox::warning(this, "Warning", "Clear all users??? ", QMessageBox::Yes ,QMessageBox::No,QMessageBox::NoButton)== QMessageBox::No)
		return;
	if(QMessageBox::warning(this, "Warning", "Clear all users??? ", QMessageBox::Yes ,QMessageBox::No,QMessageBox::NoButton)== QMessageBox::No)
		return;
...
void admin :: CheckThread2()
{
	if(binary_over)
	{
		timer->stop();
		Compile_2->setEnabled(true);	
		Team->setEnabled(true);
		Problem->setEnabled(true);
		Refresh->setEnabled(true);
		Test->setEnabled(true);
...
void admin::Compile_2_clicked()
{
    Compile_2->setEnabled(false);
    Team->setEnabled(false);
    Problem->setEnabled(false);
    Refresh->setEnabled(false);
    Test->setEnabled(false);
	...
	delete timer;
    timer = new QTimer(this);
    connect( timer, SIGNAL(timeout()),this, SLOT(CheckThread2()) );	
    QString binarypath = QDir::homeDirPath()+"/users/"+Team->currentText()+"/"+Problem->currentText();
    QString codepath = QDir::homeDirPath()+"/users/"+Team->currentText()+"/"+Problem->currentText()+".cpp" ;
    QString comm = "g++ -o " +binarypath+" "+codepath;
    char* command = new char[comm.length()+1];
    for(uint i=0;i<comm.length();i++)
	command[i]=QChar(comm.at(i));
...
void admin::txtrules_modificationChanged( bool modified)
{    
	WriteFile(path+"rules",txtrules->text());
}


void admin::ref_rules_clicked()
{
	Tab_selected("Rules");
}


void admin::SubQ_doubleClicked()
{
    if(QMessageBox::warning(this, "Warning", "Clear Submission Queue ??? ", QMessageBox::Yes ,QMessageBox::No,QMessageBox::NoButton)== QMessageBox::No)
		return;
    if(QMessageBox::warning(this, "Warning", "Clear Submission Queue ???", QMessageBox::Yes ,QMessageBox::No,QMessageBox::NoButton)== QMessageBox::No)
		return;
    WriteFile(path+"queue","");
}


void admin::ClearResult_clicked()
{
    if(QMessageBox::warning(this, "Warning", "Clear Result ???", QMessageBox::Yes ,QMessageBox::No,QMessageBox::NoButton)== QMessageBox::No)
		return;
    QFile(path+"result").remove();
}
